...
import com.cra.princess.messaging.WaterSpeedMessage;
...
public class WaterSpeedSensor implements JsonConfigurable, MessageHandler<GroundTruthMessage>, TimeStepped, Sensor {
	...
	public void update() {
		...
		if (elapsed >= messageInterval && cachedState != null) {
			...
			WaterSpeedMessage measurement = getMeasurement(cachedState);
			EventDispatcher.publishEvent(measurement);
			JmsManager.sendWaterSpeedMessage(measurement);
		}
	}
	
	private WaterSpeedMessage getMeasurement(GroundTruthMessage state) {
		WaterSpeedMessage message = new WaterSpeedMessage();
		double speed = Math.sqrt(state.surge*state.surge + state.sway*state.sway + state.heave*state.heave);
		message.speed = corruptor.corrupt(speed);
	...
	public void configure(JsonObject o) {		
        corruptor = new AdditiveSensorCorruptor("water_speed");
