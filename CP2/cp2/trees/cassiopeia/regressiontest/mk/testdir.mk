...
.if $(TESTTYPE) == "parser"
$(T).log: $(T).casp
	$(CASP) -pp $(T).casp > $@ 2>&1 || echo FAILED >> $@

.elif $(TESTTYPE) == "interp"
$(T).log: $(CASPFILE) $(T).prog
	$(CASP) $(CASPFILE) -interp $(T).prog > $@ 2>&1 || \
		echo FAILED >> $@

.elif $(TESTTYPE) == "verify"
$(T).log: $(CASPFILE.$(T)) $(T).spec $(T).prog
	$(CASP) $(CASPFILE.$(T)) -verify $(T).spec $(T).prog -o $(T).out > $@ 2>&1
	racket $(T).out >> $@ 2>&1 || \
		echo FAILED >> $@

.elif $(TESTTYPE) == "synth"
$(T).log $(T).out: $(CASPFILE.$(T)) $(T).spec
	$(CASP) $(CASPFILE.$(T)) -synth $(T).spec -o $(T).out > $@ 2>&1
	racket $(T).out >> $@ 2>&1 || \
		echo FAILED >> $@

.elif $(TESTTYPE) == "extract"
$(T).log $(T).out: $(CASPFILE.$(T)) $(T).prog
	$(CASP) $(CASPFILE.$(T)) -extract $(T).prog -o $(T).out > $@ 2>&1 || \
		echo FAILED >> $@
